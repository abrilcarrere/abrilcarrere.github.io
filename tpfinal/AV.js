...
if (this.estado == 0) {
 ...
 this.pantalla() //llamo al metodo pantalla para que se vea el arreglo de los textos
...
pantalla(i, x, y, lon, alto) {
  ...
  fill(0); 
  if (this.estado === 10) {
    fill(255); // Cambiar el color del texto en el estado 11 a blanco
    text(this.textoEstados[10], 5, 5, 580, 110);
  } else if (this.estado === 14) {
    fill(255); 
    text("TRABAJO FINAL N6\nJUEGO+AVENTURA\nCARRERE ABRIL\nPROFE: MATIAS JL", 5, 5, 580, 500);
  } else {
    text(this.textoEstados[this.estado], 5, 5, 580, 110);
  }
}
